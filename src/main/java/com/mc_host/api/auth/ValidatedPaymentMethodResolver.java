...
public class ValidatedPaymentMethodResolver implements HandlerMethodArgumentResolver {
	...
	public String resolveArgument(
		...
		String subscriptionCustomerId = paymentMethodRepository.selectPaymentMethod(paymentMethodId)
			...
			.orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Payment method %s not found".formatted(paymentMethodId)));
		...
		if (!subscriptionCustomerId.equals(userCustomerId)) {
			throw new ResponseStatusException(HttpStatus.FORBIDDEN, "User %s not authorised for payment method %s".formatted(userId, paymentMethodId));
		}
